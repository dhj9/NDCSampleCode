...
public class Client
{
	...
	{
		HttpClient httpClient = HttpClientBuilder.create().setUserAgent( "TestClient" ).build();

		try
		{
			// Make an enquiry to the airline
			//
			HttpPost request = new HttpPost( "http://iata.api.mashery.com/athena/api" );

			InputStreamReader is = new InputStreamReader(Client.class.getResourceAsStream( "/NDC_Request.xml" ) );
